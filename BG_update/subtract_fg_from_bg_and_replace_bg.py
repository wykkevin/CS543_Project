...
background_image_name = "train_img_videobg_from_video.jpg"
...
for x in range(15000, 18302):
    index = str(x)
    while len(index) < 6:
        index = '0' + index
    image_file = 'frame' + index + '.png'
    ...
    if original_frame is not None:
        ...
        gray_new_frame = scipy.signal.medfilt(gray_new_frame, kernel_size=5)
        for i in range(h):
            for j in range(w):
